...
contains
  ...
  subroutine initglobaldata_usr
    ...
    mhd_gamma=5.0d0/3.0d0       !3.0d0/2.0d0
  ...
  subroutine specialsource(qdt, ixI^L, ixO^L, iw^LIM, qtC, wCT, qt, w, x)
    ...
    double precision :: omega_normalized, omega
    double precision :: temp_v2mom(ixI^S)
    double precision :: B2(ixI^S)
    double precision :: q(ixI^S,1:ndim)
    double precision :: divQ(ixI^S)
    double precision :: alpha(ixI^S)
    double precision :: R_AU(ixI^S)
    integer :: d
    
    double precision :: L, k, R0
    
    !double precision :: alpha0, d_para, b_para, R_IB
    
    !double precision :: alpha0, gamma_in, gamma_out
    !double precision :: R_IB, R_b, s, log_IB
    ...
    Gravconst_Msun_normalized = const_G * 1d-3 * const_MSun / 1.0d3  &
       / (unit_velocity**2 * unit_length)

    !----------------------------------------------------------------------
    ! GRAVITY SOURCE TERM (unchanged)
    !----------------------------------------------------------------------
    w(ixO^S, e_)     = w(ixO^S, e_)     &
       - qdt * wCT(ixO^S, mom(1)) * Gravconst_Msun_normalized &
       / (x(ixO^S,1)**2)

    w(ixO^S, mom(1)) = w(ixO^S, mom(1)) &
       - qdt * wCT(ixO^S, rho_) * Gravconst_Msun_normalized &
       / (x(ixO^S,1)**2)

    !----------------------------------------------------------------------
    ! HEAT-FLUX SOURCE TERM (Hollweg 1974 alpha(R))
    !----------------------------------------------------------------------

    ! Convert radius from Rs to AU
    ! 1 Rs / 1 AU = 0.00465047
    R_AU(ixI^S) = x(ixI^S,1) * 0.00465047d0  
        
    !-----------------------------------------------------------------------
    ! Sigmoid plus offset profile
    !-----------------------------------------------------------------------
    L  = 1.98d0
    k  = 6.0d0
    R0 = 0.47d0 !100/215
    alpha(ixI^S) = L / (1.0d0 + exp(-k*(R_AU-R0))) + 2.0d0
    
    !-----------------------------------------------------------------------
    ! Power-law plus offset profile
    !-----------------------------------------------------------------------
    !alpha0    = 1.248d0
    !d_para    = 1.35d-3
    !b_para    = 1.92d0
    ...
    !alpha(ixI^S) = alpha0 + d_para*((R_AU(ixI^S)/R_IB)**b_para-1)
    
    !-----------------------------------------------------------------------
    ! End Power-law plus offset profile
    !-----------------------------------------------------------------------  
    !-----------------------------------------------------------------------
    ! Broken-log mean profile
    !-----------------------------------------------------------------------
    ...
    !log_IB = log(1.d0 + (R_IB/R_b)**s)

    !alpha(ixI^S) = alpha0                                      &
    !     + gamma_in * log(R_AU(ixI^S)/R_IB)                    &
    !     + (gamma_out-gamma_in)/s * (                          &
    !         log(1.d0 + (R_AU(ixI^S)/R_b)**s) - log_IB         &
    !       )
           
    !-----------------------------------------------------------------------
    ! End Broken-log mean profile
    !-----------------------------------------------------------------------    

    ! Prevent negative heat-flux coefficient
    alpha(ixI^S) = max(alpha(ixI^S), 0.0d0)

    if (maxval(alpha(ixI^S)) .ne. 0.0d0) then

      temp_v2mom(ixI^S) = 0.0d0
      B2(ixI^S)         = 0.0d0
      divQ(ixI^S)       = 0.0d0
      q(ixI^S,1:ndim)   = 0.0d0

      ! v^2 term
      do d = 1, ndim
        temp_v2mom(ixI^S) = temp_v2mom(ixI^S) + &
           wCT(ixI^S, mom(d))**2
      end do

      ! Magnetic energy density term
      B2(ixI^S) = wCT(ixI^S, mag(1))**2  &
              + wCT(ixI^S, mag(2))**2  &
              + wCT(ixI^S, mag(3))**2

      ! Heat flux vector q
      do d = 1, ndim
        q(ixI^S,d) = 3.0d0/4.0d0 * alpha(ixI^S) * (mhd_gamma - 1.0d0) * &
           ( wCT(ixI^S, e_)                                      &
           - 0.5d0 * temp_v2mom(ixI^S) / wCT(ixI^S, rho_)        &
           - 0.5d0 * B2(ixI^S) )                                 &
           * ( wCT(ixI^S, mom(d)) / wCT(ixI^S, rho_) )
      end do

      ! Divergence of heat flux
      call divvector(q, ixI^L, ixO^L, divQ)

      ! Update conserved energy
      w(ixO^S, e_) = w(ixO^S, e_) - qdt * divQ(ixO^S)
